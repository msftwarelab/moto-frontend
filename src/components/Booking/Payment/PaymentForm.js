...
class PaymentForm extends Component {
  ...
  async handleSubmit(values, dispatch) {
    ...
    if (data && data.checkReservation && data.checkReservation.status == "200") {
      ...
      Omise.setPublicKey("pkey_test_5xn33w5ko66q130ssr7");
      // console.log(this.props.baseCurrency, this.props.rates, this.props.currency, this.props.currentCurrency)
      ...
      if (values.paymentMethod === "creditCard") {
        ...
        Omise.createSource(values.paymentMethod, tokenParameters, function(statusCode, response) {
          ...
          if (statusCode >= 200 && statusCode < 300) {
            ...
            toastr.error("Oops!", "Try again or later");
          }
          // if (status == 400 && paymentType == 2) {
          //   const cardAction = await stripe.handleCardAction(paymentIntentSecret);
          //   let amount = values.total + values.guestServiceFee;
          ...
          <div className={cx(s.avatarImageGrid, cs.spaceBottom4)}>
            ...
            <div className={cx(s.textSection, "viewListingTextSectionRTL")}>
              <a
                ...
                target={"_blank"}
                className={cx(cs.commonSubTitleText, cs.siteTextColor, cs.fontWeightBold)}
              >
            ...
            {/* {getPaymentMethods && getPaymentMethods.length > 1 && (
              <>
                ...
                >
                  ...
                  <option value={3}>{formatMessage(messages.opnPayments)}</option>
                </Field>
              </>
            )} */}
            {getPaymentMethods && getPaymentMethods.length == 1 && (
              <>
                <label className="textAlignRightRTL">
                  {paymentStatus == 2
                    ? formatMessage(messages.stripeContent)
